...
function Login() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center overflow-hidden relative"
      style={{ background: "linear-gradient(135deg, #0A0407 0%, #180B11 50%, #0D0609 100%)" }}>
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] rounded-full opacity-20"
          style={{ background: "radial-gradient(circle, #C41858 0%, transparent 70%)", filter: "blur(80px)" }} />
        <div className="absolute top-1/4 right-1/4 w-[300px] h-[300px] rounded-full opacity-10"
          style={{ background: "radial-gradient(circle, #E8951B 0%, transparent 70%)", filter: "blur(60px)" }} />
      </div>
      ...
      <div
        className={`relative z-10 w-full max-w-sm mx-4 rounded-2xl border transition-all duration-1000
          ${loaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
        style={{
          background: "linear-gradient(145deg, #1A0911 0%, #120608 100%)",
          borderColor: "rgba(196, 24, 88, 0.25)",
          boxShadow: "0 25px 60px rgba(0,0,0,0.7), 0 0 40px rgba(196, 24, 88, 0.08)"
        }}
      >
        {/* Top accent line */}
        <div className="absolute top-0 left-8 right-8 h-px rounded-full"
          style={{ background: "linear-gradient(90deg, transparent, rgba(232, 149, 27, 0.6), transparent)" }} />
        ...
        <div className="px-8 pt-10 pb-8">
          ...
          <div className="flex justify-center mb-5">
            <svg
              ...
              className={`h-24 w-auto transition-all duration-1000 ease-out ${loaded ? "opacity-100 scale-100" : "opacity-0 scale-90"}`}
            >
              <defs>
          ...
          {/* Tagline */}
          <p className="text-center text-xs tracking-[0.3em] uppercase mb-8 font-semibold"
            style={{ color: "#E8951B" }}>
            Caregiver Portal
          </p>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="space-y-1">
              <input
                ...
                required
                className="w-full px-4 py-3 rounded-xl text-sm text-white placeholder-[#5A3340] transition-all duration-200 focus:outline-none"
                style={{
                  background: "rgba(255,255,255,0.05)",
                  border: "1px solid rgba(196, 24, 88, 0.2)",
                  boxShadow: "inset 0 1px 3px rgba(0,0,0,0.3)"
                }}
                onFocus={(e) => e.target.style.borderColor = "rgba(232, 149, 27, 0.5)"}
                onBlur={(e) => e.target.style.borderColor = "rgba(196, 24, 88, 0.2)"}
              />
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 rounded-xl font-semibold text-sm tracking-wide transition-all duration-300 mt-2 disabled:opacity-70"
              style={{
                background: loading
                  ? "linear-gradient(135deg, #E8951B, #C47B10)"
                  : "linear-gradient(135deg, #C41858 0%, #8B1035 100%)",
                color: "#fff",
                boxShadow: loading ? "none" : "0 4px 20px rgba(196, 24, 88, 0.4)"
              }}
            >
          ...
          <p className="text-center text-xs mt-6" style={{ color: "rgba(255,255,255,0.2)" }}>
            &copy; {new Date().getFullYear()} Hestia Homecare. All rights reserved.
          </p>
        </div>
      </div>
    </div>
